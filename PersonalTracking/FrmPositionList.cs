...
namespace PersonalTracking
{
    public partial class FrmPositionList : Form
    {
        ...
        {
            if (detail.ID == 0)
                MessageBox.Show("Please select position from table!");
            else
            {
                FrmPosition frm = new FrmPosition();
                frm.isUpdated = true;
        ...
        {
            detail.Position1 = dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString();
            detail.ID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[2].Value);
            detail.DepID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[4].Value);
            detail.OldDepartmentID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[2].Value);
        }
