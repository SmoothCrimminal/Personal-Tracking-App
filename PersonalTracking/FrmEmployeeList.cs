    ...
    public partial class FrmEmployeeList : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (detail.EmployeeID == 0)
                MessageBox.Show("Please select Employee from the table!");
            else
            {
                FrmEmployee frm = new FrmEmployee();
                frm.isUpdated = true;
        ...
        private void cmbDepartment_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboFull)
            {
                cmbPosition.DataSource = dto.Positions.Where(x => x.DepID == Convert.ToInt32(cmbDepartment.SelectedValue)).ToList();
            }
        }
        ...
        {
            ExportToExcel.ExcelExport(dataGridView1, true);
        }
    }
}
